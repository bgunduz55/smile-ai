...
export class JavaAnalyzer implements LanguageAnalyzer {
    ...
    public async analyzeFile(document: vscode.TextDocument): Promise<AnalysisResult> {
        ...
        const result = await this.analyzeJavaContent(content);
        this.analysisCache.set(filePath, result);
        return result;
    }

    private async analyzeJavaContent(content: string): Promise<AnalysisResult> {
        try {
...
public class CodeAnalyzer {
    ...
    public static void main(String[] args) {
        try {
            ...
            const javaProcess = spawn('java', ['-cp', 'javaparser-core.jar:json.jar', 'CodeAnalyzer']);
            ...
            const output = await new Promise<string>((resolve, reject) => {
                ...
                javaProcess.stderr.on('data', (data) => {
                    console.error('Java analiz hatası:', data.toString());
                });
                javaProcess.on('close', (code) => {
                    if (code === 0) {
                        resolve(result);
                    } else {
                        reject(new Error('Java analizi başarısız oldu'));
                    }
                });
            });
            ...
            console.error('Java analiz hatası:', error);
            throw error;
        }
    }
